...
class prb(Packet):
  ...
  fields_desc = [
                    ...
                    BitField("iq_user_data", None, 192)
                  ]
  def extract_padding(self, p):
        return "", p
...
class section_u(Packet):
    ...
    fields_desc = [
                    ...
                    BitField("startPrbu", None, 10),
                    BitField("numPrbu", None, 8),
                    PacketListField("prb",None,prb,count_from=lambda pkt:pkt.numPrbu)
                  ]
    def extract_padding(self, p):
        return "", p
...
class o_ran_fh_u(Packet):
    name = "o_ran_fh_u"
    fields_desc=[ 
                  PacketField("ecpriPcid","",ecpriPcid),
                  PacketField("ecpriSeqid","",ecpriSeqid),
                  PacketField("u_plane","",u_plane),
                  PacketListField("section",None,section_u)
                 ]
    def extract_padding(self, p):
        return "", p  
